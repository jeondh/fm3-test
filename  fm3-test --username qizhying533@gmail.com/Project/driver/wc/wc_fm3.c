...
static WC_IntHandlerCallback_CB *pWC_IntCallback = NULL;
...
/*!
 ******************************************************************************
 ** \brief Watch counter configuration
 **
 ** \param pConfigInfo point to clock configuration structure
 **
 ** \return None
 **
 ******************************************************************************
...
void WC_Config(WC_ConfigInfoT *pConfigInfo)
{
    uint8_t  tmpWccr = 0;
    /* Check the parameter */
    ASSERT (IS_WC_OUTPUT_CLOCK_VALID(pConfigInfo->CntClkSel));
    ASSERT (IS_FUNCTIONAL_STATE(pConfigInfo->CntIntEn));
    /* config watch counter */
    tmpWccr = FM3_WC->WCCR;
    tmpWccr &= ~(WCCR_WCIE | WCCR_CS | WCCR_WCIF);
    tmpWccr |= (pConfigInfo->CntClkSel<<2 | pConfigInfo->CntIntEn<<1);
    FM3_WC->WCCR = tmpWccr;
    FM3_WC->WCRL = pConfigInfo->CntVal;
    pWC_IntCallback = pConfigInfo->pIntHandlerCallback;
    return;
}
