...
static void ADC12_Callback(int32_t Status)
{
    ...
    switch (Status)
    {
        case Adc12_IRQSTATUS_SCAN:
            ...
            /*Set FIFO Stage*/
            ADC12_ScanStage = ADC12_FIFO_STAGE2 + 1;
...
static void ADC12_PrintResult(void)
{
    ...
    {         
        ...
        if(ADC12_Init.ADC12_DataSide == ADC12_DATA_SIDE_MSB)
        {
            ...
            ADC12_Value1 = ADC12_ScanData[ADC12_DataSize+1]>>4;   
        }
        if(ADC12_Init.ADC12_DataSide == ADC12_DATA_SIDE_LSB)
        {
            ADC12_Value = ADC12_ScanData[ADC12_DataSize];     
            ADC12_Value1 = ADC12_ScanData[ADC12_DataSize+1];                
        }	
        /*Conversion Voltage */	 
        ADC12_Value = (ADC12_Value*ADC12_Volt) / ADC12_MaxValue;
        ADC12_Value1 = (ADC12_Value1*ADC12_Volt) / ADC12_MaxValue;            
#ifdef DEBUG_MODE
        if((ADC12_ScanInfo[ADC12_DataSize] & 0x1F) == ADC12_CH7)
        {
            printf( "AN07:%d mv\n\r",ADC12_Value);
        }
        if((ADC12_ScanInfo[ADC12_DataSize] & 0x1F) == ADC12_CH8)
        {
            printf( "AN08:%d mv\n\r",ADC12_Value);
        }
        if((ADC12_ScanInfo[ADC12_DataSize+1] & 0x1F) == ADC12_CH7)
        {
            printf( "AN07:%d mv\n\r",ADC12_Value1);
        }
        if((ADC12_ScanInfo[ADC12_DataSize+1] & 0x1F) == ADC12_CH8)
        {
            printf( "AN08:%d mv\n\r",ADC12_Value1);
        }
        ADC12_DataSize += 2;
...
int32_t main(void)
{
    ...
    /*2 FIFO*/
    ADC12_Init.ADC12_FIFOStage = ADC12_FIFO_STAGE2;
	
    /* Set Irq callback function */
    ADC12_Init.ADC12_ConfigIrqCallback = ADC12_Callback;
    /* Set I/O port */
    FM3_GPIO->ADE |= 0x00000180;  /* Analog input */
    FM3_GPIO->PFR1 = 0x00000180;  /* Use P17  AN07 & P18 AN08*/ 
